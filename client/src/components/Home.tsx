...
const Home: React.FC = () => {
  ...
  const handleSaveNote = (newNote: Note): void => {
    if (editNote) {
      setNotes(
        notes.map((note: Note) => (note._id === newNote._id ? newNote : note))
      );
    } else {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {Array.isArray(notes) && notes.length > 0 ? (
          notes.map((note: Note) => (
            <div className="bg-gray-800 p-4 rounded-lg shadow-md" key={note._id}>
              ...
              <p className="text-sm text-gray-400 mb-4">
                {note.updatedAt ? new Date(note.updatedAt).toLocaleString() : 'No date'}
              </p>
          ...
          <div className="col-span-full text-center text-gray-400 py-8">
            {error ? "Error loading notes" : "No notes found"}
          </div>
        )}
      </div>
    </div>
